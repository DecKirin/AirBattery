//
//  AppleDeviceModels.swift
//  AirBattery
//
//  Model identifier → marketing name for Apple mobile devices, so the UI can show "iPhone 16 Pro"
//  instead of the raw "iPhone17,1" that lockdownd (`ProductType`) and the BLE scanner report.
//
//  Display-only on purpose: `Device.deviceModel` keeps the raw identifier, because `getDeviceIcon`
//  parses its generation number ("iPhone14,6" → 14) to choose an icon, and Nearcast peers and the
//  widget/CLI JSON snapshot all carry that same raw value.
//
//  Table source: https://gist.github.com/adamawolf/3048717
//

import Foundation

/// Raw identifier → name, copied verbatim from the source list (Simulator entries dropped).
let appleDeviceModelNames: [String: String] = [
    "iPhone1,1": "iPhone",
    "iPhone1,2": "iPhone 3G",
...
/// Case-insensitive index, built once, so an identifier that reaches us in a different case
/// ("iphone16,2") still resolves.
private let appleDeviceModelNamesLowercased: [String: String] = {
    var map = [String: String](minimumCapacity: appleDeviceModelNames.count)
    for (identifier, name) in appleDeviceModelNames { map[identifier.lowercased()] = name }
    return map
}()

/// Marketing name for a model identifier, or `nil` if it isn't one we know. Non-identifier values
/// that also live in `deviceModel` (AirPods names like "Airpods Pro 2", the Apple Pencil's "222",
/// Mac identifiers) simply miss and are left to the caller to display as-is.
func appleDeviceModelName(_ identifier: String) -> String? {
    let key = identifier.trimmingCharacters(in: .whitespacesAndNewlines)
    if let name = appleDeviceModelNames[key] { return name }
    return appleDeviceModelNamesLowercased[key.lowercased()]
}

/// What to actually put on screen for a `deviceModel`: the marketing name when we recognise the
/// identifier, otherwise the value unchanged.
///
/// Connectivity variants, which the trim below drops. "Celllular" is a typo in the source list, kept
/// here so those two Watch entries trim like the rest.
private let connectivityVariants: Set<String> = [
    "WiFi", "WiFi+Cellular", "WiFi+LTE", "GPS", "GPS+Cellular", "GPS+Celllular", "Cellular"
]

/// What to actually put on screen for a `deviceModel`: the marketing name when we recognise the
/// identifier, otherwise the value unchanged.
///
/// Watch and iPad names in the source list carry a connectivity variant ("Apple Watch Series 8 45mm
/// case (GPS+Cellular)", "iPad Pro 11 inch 4th Gen (WiFi)") that overruns the capsule's single line
/// and adds little beside the device's own name, so it's trimmed — along with the Watch " case",
/// which is filler. Anything else in the parentheses is a real distinguisher (storage size, screen
/// size, "China", "GSM+CDMA") and stays.
func displayModelName(_ model: String) -> String {
    guard let name = appleDeviceModelName(model) else { return model }
    var trimmed = name
    if let open = trimmed.lastIndex(of: "("), let close = trimmed.lastIndex(of: ")"), open < close {
        ...
        let kept = inner.components(separatedBy: ", ").filter { !connectivityVariants.contains($0) }
        if kept.isEmpty {
            // Nothing left worth showing — drop the parentheses and the space before them.
            trimmed = String(trimmed[trimmed.startIndex..<open]).trimmingCharacters(in: .whitespaces)
        } else {
            trimmed.replaceSubrange(trimmed.index(after: open)..<close, with: kept.joined(separator: ", "))
        }
    }
    if trimmed.hasSuffix(" case") { trimmed = String(trimmed.dropLast(5)) }
    return trimmed
}
